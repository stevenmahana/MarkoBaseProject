$_mod.def("/marko-widgets$6.6.2/lib/init-widgets-browser", function(require, exports, module, __filename, __dirname) { /*
...
var _addEventListener = require('/marko-widgets$6.6.2/lib/addEventListener'/*'./addEventListener'*/);
var registry = require('/marko-widgets$6.6.2/lib/registry'/*'./registry'*/);
var warp10Finalize = require('/warp10$1.3.4/finalize'/*'warp10/finalize'*/);
var eventDelegation = require('/marko-widgets$6.6.2/lib/event-delegation'/*'./event-delegation'*/);
...
function addDOMEventListeners(widget, el, eventType, targetMethodName, handles) {
    function doAddDOMEventListener(el) {
    ...
    if (Array.isArray(el)) {
        el.forEach(doAddDOMEventListener);
    } else {
        doAddDOMEventListener(el);
    }
}
    ...
    if (widget._isWidget) {
        ...
        if (domEvents) {
            ...
            for (i=0, len=domEvents.length; i<len; i+=3) {
                ...
                var eventEls = getNestedEl(widget, eventElId, document);

                // The event mapping is for a DOM event (not a custom event)
                addDOMEventListeners(widget, eventEls, eventType, targetMethodName, eventListenerHandles);
            }
